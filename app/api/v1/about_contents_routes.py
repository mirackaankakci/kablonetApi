...
@router.post("/new-content", response_model=AboutContentsCreateSchema, summary="Create New About Content (Admin Only)")
def add_about_content(
    content_data: AboutContentsCreateSchema, 
...
@router.put("/{content_id}", response_model=AboutContentsUpdateSchema, summary="Update About Content (Admin Only)")
def update_about_content(
    content_id: int, 
    content_data: AboutContentsUpdateSchema, 
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin())
):
...
@router.delete("/{about_content_id}", response_model=DeleteAboutContentsSchema, summary="Delete About Content (Admin Only)")
def deactivate_about_content(
    about_content_id: int, 
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin())
):
