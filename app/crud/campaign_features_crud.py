...
def get_campaign_features_by_id_from_db(campaign_features_id: int):
...
def update_campaign_features_in_db(campaign_features_data, campaign_features_id: int):
    db: Session = SessionLocal()
    campaign_features = db.query(CampaignFeatures).filter(CampaignFeatures.id == campaign_features_id).first()
    if not campaign_features:
        db.close()
        return None
    for key, value in campaign_features_data.dict().items():
